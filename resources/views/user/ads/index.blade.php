    ...
    <div class="row">
            ...
            <div class="tab-content" id="pills-tabContent">
              <div class="tab-pane fade show active" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                <div class="row">
                            ...
                            @if ($item->expired_date==null)
                                <div class="col-xl-4 col-md-4">
                                    ...
                                    <div class="wsus__single_property_img">
                                        <img src="{{ asset($item->images[0]) }}" alt="properties" class="img-fluid w-100">
                                    ...
                                    <div class="wsus__single_property_text">
                                        ...
                                        <a href="{{ route('ads.details',$item->id) }}" class="title w-100">{{ $item->propertyType->type.'- '. config('constants.ad_transactions.'.$item->transactions) }}</a>
                                        <ul class="d-flex flex-wrap justify-content-between">
                                            <li><i class="fal fa-bed"></i> {{ $item->total_bedroom }} {{ $websiteLang->where('lang_key','bed')->first()->custom_text }}</li>
                                            <li><i class="fal fa-shower"></i> {{ $item->total_bathroom }} {{ $websiteLang->where('lang_key','bath')->first()->custom_text }}</li>
                                            <li><i class="fal fa-draw-square"></i> {{ $item->built_area }} {{ $websiteLang->where('lang_key','sqft_s')->first()->custom_text }}</li>
                                        </ul>
                                        <div class="wsus__single_property_footer d-flex justify-content-between align-items-center">
                                            <a href="{{ route('ads-listing',['page_type' => 'list_view','property_type' => $item->propertyType->id]) }}" class="category">{{ $item->propertyType->type }}</a>
                                        </div>
                                    </div>
                                    </div>
                                </div>
                    ...
                    @else
                    <div class="col-12 text-center">
                        <h3 class="text-danger">Ads not found</h3>
                    </div>
                    @endif
      ...
      {{-- <div class="col-xl-4">
        <div class="wsus__search_property" id="sticky_sidebar">
          ...
          <form method="GET" action="{{ route('ads-listing') }}">
            ...
            <div class="wsus__single_property_search">
              ...
              <select class="select_2" name="property_type">
                ...
                @foreach ($propertyTypes as $key=>$property_type_item)
                    <option value="{{ $key }}">{{ $property_type_item }}</option>
                @endforeach
            </select>
            </div>
                    ...
                    <div class="accordion-body">
                        @foreach ($aminities as $key=>$aminity)
                            <div class="form-check">
                                <input name="aminity[]" class="form-check-input" type="checkbox" value="{{ $key }}" id="flexCheckDefault-{{ $key }}">
